...
- **Corregido (PLT-020, 2026-09-05): la válvula miraba al esquema equivocado.** La regla real (PLT-012 regla 2) exige anonimizar en vez de borrar si el usuario tiene historial transaccional. `seg_fn_eliminar_cuenta()` vigilaba `to_regclass('comun_facturacion.fac_transaccion_pago')`, tabla que sigue sin existir — pero desde `20260905000001` **sí hay historial transaccional real** en `comun_comercio.com_transaccion_pago`, que la función no miraba. El resultado eran dos fallos distintos: un usuario **con** pagos, suscripción o proforma no podía darse de baja —la transacción abortaba por violación de clave foránea contra `comun_comercio`, con un error ininteligible en vez del mensaje que PLT-012 promete—; y un usuario **sin** nada de eso pero con billetera sí se borraba, arrastrando su saldo de bono de convenio, porque `com_billetera.wlt_usuario_id` era `on delete cascade`. El segundo caso era el único que perdía datos. La función ahora consulta `com_transaccion_pago` y `com_suscripcion` de verdad y **anonimiza** cuando hay historial; las FK de `comun_comercio` hacia `seg_usuario` pasaron a `on delete set null` y la billetera a `on delete restrict`. Ver `20260905000002_fix_baja_cuenta_historial_comercio.sql`.
...
| `comun_facturacion` | ❌ Pendiente | — | Los pagos viven hoy en `comun_comercio` (`com_transaccion_pago`) |
