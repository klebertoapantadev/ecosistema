...
17. **Construcción Dinámica de Menú Sidebar por Perfil y Botón Único de Salida (`NavegacionSidebar.tsx`):**
    - **Renderizado Dinámico de Paneles:** Las opciones de la barra lateral se generan iterando la lista de paneles configurados (`panelesSidebar`). Si un panel tiene **1 o más widgets asignados** para el perfil activo (ej. `CLIENTE`, `ABOGADO`, `ADMINISTRADOR`), se muestra como **Enlace Activo**. Si tiene **0 widgets asignados**, se muestra como **Inerte con Badge `[pronto]` (Próximamente)**.
    - **Botón Único de Cerrar Sesión:** Se unifica la acción de salida a **1 sola opción visible** situada al final del menú lateral (`.panel-nav-links`), eliminando duplicados en la tarjeta inferior de perfil de usuario.
