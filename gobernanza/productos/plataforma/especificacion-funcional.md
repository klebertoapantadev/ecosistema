...
36. **Funcionalidades de Eliminación Total & Reset del Sistema para Pruebas desde Cero (`seg_fn_superadmin_eliminar_usuario`, `seg_fn_superadmin_resetear_sistema`, `20260907000001_reset_sistema_solo_usuarios_y_datos.sql`):**
    ...
    - **Reset Seguro del Sistema Aislado Exclusivamente por Negocio (Tranqi `TRANQ`):** Se garantiza que el Reset Master purga **únicamente** los usuarios de prueba, perfiles asignados y datos operacionales (`trq_caso`, `trq_expediente`, `trq_billetera_documento`, `trq_lead_crm`, transacciones de comercio y notificaciones) del negocio indicado (`TRANQ`), conservando el 100% de las configuraciones del negocio (`cfg_negocio`), credenciales SMTP (`cfg_smtp`), términos y consentimientos (`seg_termino_condicion`), plantillas de contratos (`trq_plantilla_contrato`), catálogo comercial unificado (`com_producto`) y el catálogo maestro de perfiles (`seg_perfil`). Queda estrictamente aislado para no afectar jamás a los demás negocios (`FFH`, `TNK`, `MRG`).
    - **Accesibilidad y Microcopia de Eliminación en Widgets de Perfiles y Tablas:** El botón de eliminar cuenta y el botón de retirar/eliminar perfiles en `AdministracionPerfilesWidget`, `ConsultaUsuariosPerfilesWidget` y `FilaUsuario` implementan modo responsive `.btn-responsive-accion` con columna fija `sticky` para ser 100% accesibles en dispositivos móviles sin desbordes.
...
44. **Pre-configuración Obligatoria por Defecto de Nuevos Widgets y Paneles por Rol:**
    - **Cero Widgets Huérfanos:** Cada widget creado o integrado en el ecosistema DEBE registrarse en `comun_seguridad.seg_widget` y pre-asignarse de forma automática en `comun_seguridad.seg_rol_widget`, `AdministracionPerfilesWidget.tsx` (`PERFILES_INICIALES`), `NavegacionSidebar.tsx` y `PanelDinamicoModular.tsx`.
    - **Distribución Funcional por Defecto:**
      - *Herramientas Digitales, Firma PDF, Billetera OCR/TTL, Catálogo Comercial:* Asignados a `panel_herramientas` para `CLIENTE`, `ABOGADO`, `OPERADOR`, `ADMINISTRADOR`, `SUPERADMIN`.
      - *CRM Jurídico, Directorio 360°, Alta Asistida, Aprobación de Socios, Solicitudes, Historial de Pagos, Notificaciones y Términos:* Asignados a `panel_administrar` para `OPERADOR`, `ADMINISTRADOR`, `SUPERADMIN` (y `ABOGADO` para su CRM).
      - *Configuración de Negocio, SMTP Vault, Pasarela Payphone, Agentes IA, Perfiles:* Asignados a `panel_configuracion` para `ADMINISTRADOR` y `SUPERADMIN`.
    - **Autonomía Operativa Inmediata:** Los administradores mantienen la facultad de reorganizar o transferir widgets posteriormente, pero el acceso inicial queda operativo y accesible de inmediato por defecto desde el código y la base de datos sin requerir ninguna configuración manual.
