...
| `20260729000001_otp_registro_correo` | Verificación de correo por OTP de 6 dígitos, reemplaza el magic link de Supabase Auth (remitente compartido del proyecto, no del negocio). `seg_usuario.usu_correo_verificado_en`, tabla `seg_otp_correo` (`otp_codigo_hash` vía `bcrypt`, expira, intentos), `seg_fn_generar_otp_registro()`/`seg_fn_verificar_otp_registro()`. Google OAuth se marca verificado automático. El envío real usa `@eco/notificaciones` (SMTP propio de cada negocio, no Supabase Auth) — ver ADR de remitente por negocio en la especificación de identidad. |
| `20260729000002_fix_otp_registro_pgcrypto_schema` | **Fix**: `gen_salt(unknown) does not exist` — con `set search_path = ''` las funciones de `pgcrypto` deben calificarse como `extensions.crypt`/`extensions.gen_salt` (confirmado `extensions` como esquema de instalación de la extensión en este proyecto). |
| `20260730000001_recuperacion_contrasena` | Restablecer contraseña con remitente por negocio (Opción A, decidida sobre la nativa de Supabase Auth). Tabla `seg_recuperacion_correo` (token de 256 bits vía `extensions.gen_random_bytes`, hash SHA-256 — no bcrypt, se necesita búsqueda indexada por igualdad, no solo verificación), `seg_fn_solicitar_recuperacion()` (cooldown 60s, expira 30min). El cambio de clave real ocurre en la Edge Function `restablecer-contrasena` (único lugar permitido para `service_role`, ver `supabase/functions/README.md`). |
| `20260730000002_otp_registro_expira_15_minutos` | **Fix**: expiración de 10 minutos era insuficiente frente a la latencia real de revisar el correo — subida a 15. |
| `20260730000003_recuperacion_service_role_grant` | **Fix**: mismo patrón de `20260728000003/000007` — `service_role` no tiene `USAGE`/`SELECT`/`UPDATE` automático sobre `comun_seguridad`/`seg_recuperacion_correo`; la Edge Function fallaba con `permission denied for schema` (silenciado en el cliente, diagnosticado con `debug` temporal en la respuesta). |
| `20260730000004_recuperacion_anon_schema_usage` | **Fix**: mismo patrón otra vez, ahora para `anon` — `seg_fn_solicitar_recuperacion()` es la primera función de `comun_seguridad` invocada por un usuario sin sesión; nadie había otorgado `USAGE on schema comun_seguridad to anon`. El correo de recuperación no se enviaba pero el mensaje genérico de éxito lo ocultaba (por diseño, para no filtrar existencia de cuenta) — se agregó `console.error` del `error` descartado para no repetir el diagnóstico a ciegas. |
| `20260730000005_aud_fn_listar_auditoria_negocio` | PLT-011 regla 2 (DataGrid transversal, ver [ADR-0004](./adr/0004-datagrid-transversal.md)). Primer `returns table(...)` del proyecto: unifica auditoría del esquema propio del negocio con eventos de identidad de `comun_seguridad` (`seg_usuario`, `seg_membresia`, `seg_otp_correo`, `seg_recuperacion_correo`) acotados a miembros con membresía activa en ese negocio. `security definer`, autoriza con `seg_fn_es_admin_negocio()` porque la RLS existente de `aud_registro` no cubre `comun_seguridad` para roles de negocio. Consumida por `@eco/auditoria` en `/panel/auditoria`. |
| `20260816000001_asignar_widgets_terminos_contratos_operador` | Registro exhaustivo del catálogo de widgets modulares en `comun_seguridad.seg_widget` para los 4 negocios (`tranqi`, `fastfix`, `tinkay`, `margaritas`) y asignación transversal en `seg_rol_widget` para los perfiles `OPERADOR`, `ADMINISTRADOR` y `SUPERADMIN` (incluyendo Términos, Contratos & Beneficios, Vacantes y Postulaciones). |

Tipos TypeScript regenerados en cada migración que cambia columnas — ver `packages/db/src/tipos-generados.ts` (`20260727000009` solo reescribe el cuerpo de una función, no requiere regeneración; lo mismo aplica a `20260728000003/4/5/7` y `20260730000002/3/4`, que solo tocan funciones/políticas/storage/grants).
