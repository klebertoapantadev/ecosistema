-- Migración: 20260925000006_fix_asignar_quitar_perfil_auditoria_y_mem_rol.sql
-- Descripción: Actualiza mem_rol y seg_membresia_perfil registrando trazabilidad y auditoría completa en asignación y revocación de perfiles.
...
BEGIN
  ...
  ELSE
    ...
    SET mem_rol = upper(p_perfil), mem_estado = 'ACTIVO', mem_actualizado_en = now()
    WHERE mem_id = v_membresia_id;
  END IF;

  -- Vincular en seg_membresia_perfil con actualización explícita para disparar auditoría
...
BEGIN
  ...
  IF v_membresia_id IS NOT NULL THEN
    ...
    UPDATE comun_seguridad.seg_membresia
    SET mem_rol = COALESCE(v_rol_remanente, 'CLIENTE'), mem_actualizado_en = now()
    WHERE mem_id = v_membresia_id;
  END IF;
END;
$$;
